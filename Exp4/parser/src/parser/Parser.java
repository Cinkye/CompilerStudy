	   ...
	   Stmt stmt() throws IOException {
		  ...
		  case Tag.SWITCH:
			Switch switchnode = new Switch();
		     ...
		     match(Tag.SWITCH); match('('); Id id1 = top.get(look); 
		     move(); match(')'); match('{');
		     Default defaultnode = null;
		     while(look.tag==Tag.CASE) {
			     Case casenode = new Case();
			     match(Tag.CASE);
		    	 String id2 = look.toString(); move();
		    	 match(':');
		    	 s = stmt();
		    	 casenode.init(id1,id2,s);
		    	 switchnode.store(casenode);
		     }
		     if(look.tag==Tag.DEFAULT) {
		    	 match(Tag.DEFAULT); match(':'); s1=stmt();
		    	 defaultnode = new Default(s1);
		     }
		     switchnode.init(id1,defaultnode);
		     Stmt.Enclosing = savedStmt;  // reset Stmt.Enclosing
		     return switchnode;
